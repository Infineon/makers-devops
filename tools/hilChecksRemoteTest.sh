...
# Parse options
TEMP=$(getopt -o s:d:h --long source:,destination:,remote:,help,projectYAML:,userYAML: -n "$0" -- "$@")
if [ $? != 0 ]; then
  echo "Terminating..." >&2
  usage
fi
...
tar -czf - -C "$PARENT_DIR_SOURCE" "$BASE_NAME_SOURCE" | ssh "$remote_user_machine" "tar -xpzf - -C $PARENT_DIR_DEST" || {
...
ssh "$remote_user_machine" <<EOF
  ...
  echo "\$processed_checks" | while read -r check; do
      ...
      hilChecks.py --projectYAML "$project_yaml" --userYAML "$user_yaml" --runCheck "\$check" --dockerTag=latest 2>&1 | tee -a output.log
