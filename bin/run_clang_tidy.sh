...
usage() {
  ...
  echo "          [--header-filter] <Header filter regex for headers to be processed.>  [-o | --output-dir] <output directory>  [--quiet] <Quiet.>  [--system-headers] <Process system headers.>  [--use-color] <Use color.>  [--use-shell-color] <Use color in terminal.>  [--warnings-as-errors]  <Upgrade warnings to errors for specified checks.> " 1>&2;
  exit 1;
}


# This requires GNU getopt.
TEMP=`getopt -o o: --long checks:,config-file:,export-fixes:,extra-arg:,fcolor-diagnostics,file-prefix:,fix,header-filter:,output-dir:,quiet,system-headers,use-color,use-shell-color,warnings-as-errors: -n "$0" -- "$@"`
...
for pattern in $*; do
    ...
    for file in $file_list; do
        ...
        $unbuffer clang-tidy $checks $config_file $export_fixes $extra_arg $fix $header_filter $quiet $system_headers $use_color $warnings_as_errors $file -- 2>&1 | tee $output_dir/$file_prefix.$file_base.log
