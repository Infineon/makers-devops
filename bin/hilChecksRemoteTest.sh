...
if [ "$skip_copy" = false ]; then
  ...
  tar -czf - -C "$PARENT_DIR_SOURCE" "$BASE_NAME_SOURCE" | ssh "$remote_user_machine" "tar --no-same-owner --no-same-permissions -xzf - -C $PARENT_DIR_DEST --checkpoint=1000 --checkpoint-action=dot" || {
